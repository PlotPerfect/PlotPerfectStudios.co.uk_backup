...
async function createTextImage(text, font, color){
  return new Promise((resolve) => {
    ...
    // Set canvas size
    c.width = maxWidth + 40; // padding
    c.height = lines.length * lineHeight + 40;
    ...
    lines.forEach((line, index) => {
      const y = (index + 0.5) * lineHeight + 20; // center each line
...
async function init(){
  ...
  // Reduced motion: just show the text image (still looks premium)
  if (prefersReducedMotion()){
    const img = await createTextImage(CONFIG.text, CONFIG.font, CONFIG.textColor);
    container.innerHTML = `<img src="${img.src}" alt="Coming Soon" />`;
    return;
  }
  ...
  // Build particle positions from text
  const img = await createTextImage(CONFIG.text, CONFIG.font, CONFIG.textColor);
...
init().catch((err) => {
  // Fallback to text image if WebGL fails
  createTextImage(CONFIG.text, CONFIG.font, CONFIG.textColor).then(img => {
    const container = document.getElementById("particleLogo");
    if (container){
      container.innerHTML = `<img src="${img.src}" alt="Coming Soon" />`;
    }
  });
  console.error("Particle text init failed:", err);
});