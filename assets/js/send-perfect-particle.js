...
const CONFIG = {
  imageUrl: "/assets/images/send perfect logo.svg",
...
async function init(){
  ...
  if (prefersReducedMotion()){
    container.innerHTML = `<img src="${CONFIG.imageUrl}" alt="Send Perfect logo" />`;
    return;
  }
  ...
  // Build particle positions from image
...
init().catch((err) => {
  ...
  const container = document.getElementById("particleLogo");
  if (container){
    container.innerHTML = `<img src="/assets/images/send perfect logo.svg" alt="Send Perfect logo" />`;
  }
